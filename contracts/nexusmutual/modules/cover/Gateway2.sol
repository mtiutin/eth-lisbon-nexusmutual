...
contract Gateway2 is IGateway, MasterAware {
  ...
  function changeDependentContractAddress() override public {
  ...
  function initializeDAI() external {
    //kovan
    DAI = 0x5C422252C6a47CdacF667521566Bf7bD5b0d769B;
  ...
  function buyCoverNXM (
    ...
    // CoverType coverType,
    bytes calldata data
  ) external override onlyMember whenNotPaused returns (uint) {
...

    {
      ...
      quotation.makeCoverUsingNXMTokens(
          coverDetails,
          coverPeriod,
    ...
    emit CoverBought(coverId, msg.sender, contractAddress, coverAsset, sumAssured, coverPeriod, CoverType.SIGNED_QUOTE_CONTRACT_COVER, data);
    return coverId;
  }
  ...
  function getCurrencyFromAssetAddress(address asset) public override view returns (bytes4) {
